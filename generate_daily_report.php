...
// Fetch reservations for today - Grouped by user and dates
$sql = "SELECT GROUP_CONCAT(r.id) as ids, GROUP_CONCAT(CONCAT(ro.type, ' (', r.room_id, ') - ', f.name) SEPARATOR '\n') as room_info, 
        ...
        WHERE DATE(r.checkin_date) = ? OR DATE(r.checkout_date) = ?
...
$pdf->Cell(0, 10, utf8_decode('INDET Hotel - Reporte Diario'), 0, 1, 'L');
$pdf->SetXY(60, 25);
...
$pdf->SetTextColor(0, 51, 102);
$pdf->Cell(0, 10, utf8_decode('Reservas del Día'), 0, 1, 'L');
$pdf->Ln(5);
...
if (empty($reservations)) {
    $pdf->SetFont('Arial', '', 12);
    $pdf->SetTextColor(0, 0, 0);
    $pdf->Cell(0, 10, utf8_decode('No hay reservas para el día de hoy.'), 0, 1, 'L');
} else {
    // Table headers
...
$pdf->Output('D', 'reporte_diario_INDET_' . date('Y-m-d') . '.pdf');
?>